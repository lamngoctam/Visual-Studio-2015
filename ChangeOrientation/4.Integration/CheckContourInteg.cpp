...
void CheckContour(int ImageNum, Model& Integ,vector<cv::Mat>& Silhouette, Cpixel*** vertexII, CMatrix* ART) {
	...
	cout<<"Larget_boundingbox_contour.txt\n";
	...
	for (auto h(0); h < ImageNum; h++) {
		...
		for (auto j(0); j < Integ.SurfVoxels.size(); j++) {
			Integ.SurfVoxels[j].SetVertexWorld_Rotated_PCA(Angle_.Orientation_Angle,vertexW);
		...
		for (auto n(0); n < PointEnd.size(); n++) {
			line(drawing, PointStart[n], PointEnd[n], Scalar(225, 225, 225), 0.01, 1, 0);
		}
	...
	//	imshow("Contours", drawing);
		...
		imwrite("../../data/input/newzebra/countour_voxel/octreedepth5/0/contour_voxel" + to_string(h) + ".bmp", drawing);
