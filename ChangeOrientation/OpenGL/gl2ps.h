//############################################################################
//epsやsvgを出力するために必要なファイル
//OpenGLの描画をベクトルフォーマットepsとして吐き出せる
//吐き出したepsはtexの論文などにそのまま使えるし、Adobe Illustratorで編集もできる
//吐き出したsvgはInkscapeで編集できる
//ソースコードはGL2PSというソフトをコピペ		http://geuz.org/gl2ps/
//GLに描画されている立体物を平面に投影する処理を行っている。
//物体の交線計算はしないため、前後関係が怪しいことがよくある。
//つまり、２つの物体が交差しているときは、奥の物体が手前に表示されたりする。
//これはバグではなく仕様なのでどうしようもない。
//############################################################################
